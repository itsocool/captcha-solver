...
이 문서는 코드를 빠르게 이해하고 생산적으로 작업하기 위한 핵심 정보만 간결하게 정리합니다. 변경 전후에는 `web.core.engine`의 학습·일괄 평가 함수를 참고하세요.
...
- 일괄 평가: 웹 `/predict` 또는 `web.core.engine.batch_predict_model()`.
...
- 단건 CLI: `aso-ai` / `python -m aso_ai` (`aso_ai/cli.py`).
...
1. 작은 변경(예: 전처리/threshold 실험) 시 웹 `/predict` 또는 `web.core.engine.batch_predict_model()`로 검증
