...
- 모델 파일: PyTorch 기반 모델은 `captcha_data/<id>/<rev>/model/model.pt` (state dict), ONNX 는 같은 디렉터리의 `model.onnx` 입니다. TorchScript(`model_jit.pt`)와 Keras(`weights.keras`)는 더 이상 사용하지 않습니다.
...
- 레이블 처리와 모델 저장 규약은 PyTorch 중심입니다. 모델 저장은 `model.pt`를 기본으로 하며, Rust·Java 런타임 배포용으로 `model.onnx`를 함께 내보냅니다.
...
- 모델 저장 규약: PyTorch 모델은 `model.pt`를 기본으로 사용합니다.
...
3. 모델 배포는 `model.pt` 또는 `model.onnx` 형식을 사용
