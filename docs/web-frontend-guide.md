...
| `data_source.js` | `data_source.html` (`/data-source`) | 캡차/리비전 분리 셀렉트(리비전은 템플릿이 심은 `#data-source-targets` JSON으로 캡차별로 채우고 가장 높은 리비전을 기본 선택), 응답 형식(`content_type` image/html/json) 셀렉트에 따라 셀렉터 칸의 라벨·힌트·잠금을 바꾸는 `syncSelectorUi()`, 수집 SSE(`content_type`/`delay_ms` 포함; 추가 클릭 시 `saveCurrentParams()` 먼저 호출), draft 갤러리(라벨 없는 `draft-*` 파일은 빈 캡션+placeholder), 이미지 표시 크기에 맞춘 인라인 라벨링 입력, 수동 저장 이력의 하단 왼쪽 체크 배지, "예측으로 이름 변경" 버튼(`/data-source/auto-label/stream` SSE, 신뢰도 기준 0.92/0.86 입력, 소수점 두 자리·구간별 색상, 신뢰도→생성순 정렬, DB 예측 기록 조회·신뢰도 계산 버튼, 캡션 제자리 갱신) |
