...
.PARAMETER Clean
	build/ 를 지우고 처음부터. build.ps1 로 그대로 전달된다.
...
$packages = Join-Path $root '..\..\packages\WinConsoleApp'
New-Item -ItemType Directory -Force $packages | Out-Null
Copy-Item (Join-Path $root 'config.json') $packages -Force

Write-Host ''
Get-ChildItem (Join-Path $buildDir 'captcha-solver-*.exe') | ForEach-Object {
	Copy-Item $_.FullName $packages -Force
	Write-Host "완료 : $(Join-Path (Resolve-Path $packages) $_.Name)  ($([math]::Round($_.Length / 1MB, 1)) MB)" -ForegroundColor Green
}
