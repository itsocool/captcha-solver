...
#
# 환경변수
#   PORT           서버 포트 (기본 5000 — core/config.py 의 web_port 와 맞춤)
...
cmd_start() {
    ...
    command -v "$UVICORN" >/dev/null 2>&1 \
        || die "'${UVICORN}' 을 찾을 수 없습니다. 'uv sync' 로 설치하거나 UVICORN 환경변수로 경로를 지정하세요."
    ...
    local -a reload_args=()
    if [[ $RELOAD == 1 ]]; then
        # 저장소 전체를 감시하면 captcha_data 수만 장까지 물고 늘어진다. 앱 코드만 본다.
        reload_args=(--reload --reload-dir "$APP_DIR")
    fi

    log "기동합니다 (포트 ${PORT}, reload=${RELOAD}) ..."
    : > "$LOG_FILE"

    # setsid 로 새 프로세스 그룹에 띄운다. --reload 는 워커를 자식으로 fork 하므로
    # 그룹 단위로 시그널을 보내야 자식까지 함께 정리된다.
    # cd 대상은 APP_DIR 이 아니라 저장소 루트다 — db_path 등이 './db/...' 상대경로다.
    cd "$REPO_ROOT"
    setsid "$UVICORN" web.app:app \
...
# logs           마지막 50줄
# logs 200       마지막 200줄
# logs -f        따라가기 (Ctrl-C 로 빠져나옴)
cmd_logs() {
    [[ -f $LOG_FILE ]] || die "로그 파일이 없습니다 (${LOG_FILE}). 아직 기동한 적이 없습니다."

    case "${1-}" in
        -f|--follow|follow) tail -f "$LOG_FILE" ;;
        "")                 tail -n 50 "$LOG_FILE" ;;
        *[!0-9]*)           die "logs 인자는 줄 수 또는 -f 입니다: $1" ;;
        *)                  tail -n "$1" "$LOG_FILE" ;;
    esac
}
...
사용법: $(basename "$0") {start|stop|restart|status|logs}
  ...
  logs [N|-f]  로그 마지막 N줄 (기본 50), -f 는 따라가기

환경변수: PORT(기본 ${PORT}) HOST(${HOST}) RELOAD(${RELOAD})
